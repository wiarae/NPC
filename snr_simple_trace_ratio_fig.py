...
PALETTE = {
    # 예시 그림 느낌: 진한 검정, 하늘색, 머스터드
    "BASE":       "#111111",
    "UNTARGETED": "#5EB0E5",
    "UNTARGETED": "#5EB0E5",   # 오타 대비
    "TARGETED":   "#E6AA00",
}
...
def main():
    ...
    ap.add_argument("--roles", type=str, default="BASE,TARGETED,UNTARGETED",
                help="Negatives의 순서와 정렬 없이 그대로 매핑할 역할들(예: 'BASE,TARGETED,UNTARGETED'). 길이가 모자라면 순환/보충, 길면 잘림.")
    ...
    for i, neg_arg in enumerate(args.negatives):
        ...
        role = role_of_negative(neg)  # ★ 역할 결정
    ...
    for i, r in enumerate(results):
        ...
        if role == "TARGETED":
            ...
            labels.append("BASE")
    # roles = [r["role"] for r in results]           # 색상 결정 키(역할 고정)
    # # 시리즈 표시는 구분이 되도록 구성 (원하는 텍스트로 자유롭게)
    # labels = []
    # for r in results:
    #     if r["role"] == "BASE":
    #         labels.append("BASE")
    #     elif r["role"] == "UNTARGETED":
    #         labels.append("UNTARGETED (uncond)")
    #     else:
    #         # TARGETED는 무엇으로 타겟했는지 음수 프롬프트를 붙여 구분
    #         labels.append(f"TARGETED: {r['negative']}")

    xs = list(range(1, args.steps+1))

    # labels = [r["negative"] for r in results]
    # if args.labels:
    #     labels = [s.strip() for s in args.labels.split(",")]
    #     # 개수 안 맞으면 자동 보정
    #     if len(labels) != len(results):
    #         labels = (labels + [r["negative"] for r in results])[0:len(results)]
